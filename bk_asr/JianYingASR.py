...
class JianYingASR(BaseASR):
    ...
    def submit(self) -> str:
        ...
        if resp_json.get("ret") != "0":
            raise RuntimeError(
                f"剪映任务提交失败: {resp_json.get('errmsg', '未知错误')} (ret={resp_json.get('ret')})"
            )

        query_id = resp_json.get("data", {}).get("id")
        if not query_id:
            raise RuntimeError(f"剪映任务提交未返回有效 ID: {resp_json}")
        return query_id
    ...
    def _run(self, callback=None):
        if callback:
            callback(20, "正在上传音频...")
        logging.info("剪映 ASR: 正在上传音频...")
        self.upload()

        if callback:
            callback(50, "提交识别任务...")
        logging.info("剪映 ASR: 正在提交识别任务...")
        query_id = self.submit()

        if callback:
            callback(60, "等待识别结果...")
        logging.info(f"剪映 ASR: 正在轮询查询识别结果 (ID: {query_id})...")

        # 轮询获取识别结果
        resp_data = None
        for attempt in range(60):
            resp_data = self.query(query_id)
            if resp_data.get("ret") == "0" and "data" in resp_data:
                # 检查是否已包含 utterances
                if "utterances" in resp_data["data"]:
                    break
            elif resp_data.get("ret") != "0":
                raise RuntimeError(
                    f"剪映查询失败: {resp_data.get('errmsg', '未知错误')} (ret={resp_data.get('ret')})"
                )
            time.sleep(1)

        if not resp_data or "data" not in resp_data or "utterances" not in resp_data["data"]:
            raise TimeoutError("剪映 ASR 识别超时或未返回有效字幕数据")

        if callback:
            callback(100, "转录完成")
        logging.info("剪映 ASR: 识别完成")
        return resp_data

    def _make_segments(self, resp_data: dict) -> list[ASRDataSeg]:
        if "data" not in resp_data or "utterances" not in resp_data["data"]:
            raise ValueError(f"无法从响应中解析字幕数据: {resp_data}")
        ...
        if self.need_word_time_stamp:
            ...
            return [
                ASRDataSeg(u["text"].strip(), u["start_time"], u["end_time"])
                for u in utterances
            ]
    ...
    def _generate_sign_parameters(
        self, url: str, pf: str = "4", appvr: str = "6.0.0", tdid=""
    ) -> Tuple[str, str]:
        ...
        data = {
            ...
            "pf": pf,
            "appvr": appvr or self.appvr,
            "tdid": self.tdid,
        }
        get_sign_url = "https://asrtools-update.bkfeng.top/sign"
        try:
            response = requests.post(get_sign_url, json=data, timeout=10)
            response.raise_for_status()
            response_data = response.json()
            sign = response_data.get("sign")
            if not sign:
                raise ValueError(f"签名接口未返回 sign: {response_data}")
        except requests.exceptions.RequestException as e:
            raise RuntimeError(f"获取剪映签名失败 (网络异常): {e}")
        except ValueError as ve:
            raise RuntimeError(f"获取剪映签名失败: {ve}")
    ...
    def _upload_sign(self):
        ...
        sign, device_time = self._generate_sign_parameters(
            url="/lv/v1/upload_sign", pf="4", appvr=self.appvr, tdid=self.tdid
        )
        headers = self._build_headers(device_time, sign)
        ...
        if login_data.get("ret") != "0" or "data" not in login_data:
            raise RuntimeError(
                f"获取剪映上传凭证失败: {login_data.get('errmsg', login_data)}"
            )
    ...
    def _upload_auth(self):
        ...
        request_parameters = (
            ...
            f"&SpaceName=lv-mac-recognition&Version=2020-11-19&s=5y0udbjapi"
        )
